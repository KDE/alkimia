...
bool AlkOnlineQuote::launchWebKitCssSelector(const QString &_symbol, const QString &_id,
                                             const QString &_source)
{
    ...
    connect(d->m_webView, SIGNAL(loadFinished(bool)), this,
    ...
    disconnect(d->m_webView, SIGNAL(loadFinished(bool)), this,
               SLOT(slotLoadFinishedCssSelector(bool)));

    return !(d->m_errors & Errors::URL || d->m_errors & Errors::Price
             || d->m_errors & Errors::Date || d->m_errors & Errors::Data);
}

bool AlkOnlineQuote::launchWebKitHtmlParser(const QString &_symbol, const QString &_id,
                                            const QString &_source)
{
    if (!initLaunch(_symbol, _id, _source)) {
        return false;
    ...
    delete d->m_eventLoop;
    disconnect(d->m_webView, SIGNAL(loadStarted(bool)), this, SLOT(slotLoadStarted(bool)));
    disconnect(d->m_webView, SIGNAL(loadFinished(bool)), this, SLOT(slotLoadFinishedHtmlParser(
                                                                        bool)));
...
bool AlkOnlineQuote::launchNative(const QString &_symbol, const QString &_id,
                                  const QString &_source)
{
    bool result = true;
    if (!initLaunch(_symbol, _id, _source)) {
    ...
    if (url.isLocalFile()) {
        ...
        if (KIO::NetAccess::download(url, tmpFile, 0)) {
            ...
            if (f.open(QIODevice::ReadOnly)) {
                ...
                d->m_webView->setContent(quote.toLocal8Bit());
...
bool AlkOnlineQuote::parseDate(const QString &datestr)
{
    if (!datestr.isEmpty()) {
        ...
        try {
            ...
            emit error(i18n("Unable to parse date %1 using format %2: %3").arg(datestr,
                                                                               dateparse.format(),
                                                                               e.what()));
